...
lazy_load_segment (struct page *page, void *aux) {
	...
	struct lazy_info *lazy_info = (struct lazy_info *) aux;

	if(file_read_at(lazy_info->file, page->frame->kva, lazy_info->read_bytes, lazy_info->ofs) != (int)lazy_info->read_bytes){
...
/* 파일에서 오프셋 OFS에서 시작하는 세그먼트를 UPAGE 주소에 로드합니다.
 * 총 READ_BYTES + ZERO_BYTES 바이트의 가상 메모리가 초기화되며, 
 * 이 작업은 다음과 같이 이루어집니다:
 *
 * - UPAGE에서 READ_BYTES 바이트는 FILE에서 오프셋 OFS부터 읽어옵니다.
 * - UPAGE + READ_BYTES에서 ZERO_BYTES 바이트는 제로화됩니다.
 *
 * 이 함수에서 초기화되는 페이지는 WRITABLE이 true이면 사용자 프로세스에서 쓸 수 있어야 하며,
...
setup_stack (struct intr_frame *if_) {
	...
	/* TODO: stack_bottom 위치에 스택을 매핑하고 즉시 페이지를 클레임하세요.
	 * TODO: 성공했다면, rsp 레지스터를 적절히 설정하세요.
	 * TODO: 이 페이지가 스택이라는 것을 명시해야 합니다. */
